  ...
  if (raw.trim().startsWith("{")) {
    try {
      ...
      return {
        ...
        serviceId: String(j.service_id || j.serviceId || process.env.WEATHERKIT_SERVICE_ID || process.env.APPLE_NATIVE_BUNDLE || "").trim(),
      };
    } catch (_) { /* fall through to PEM form */ }
  }
  // Form A: raw PEM + separate id vars. The `sub` (Service ID) falls back to
  // APPLE_NATIVE_BUNDLE (the app's bundle id, already set) so no new env var is
  // needed when WeatherKit is enabled on that App ID — respects the 20-var cap.
  let pem = raw;
  if (pem && pem.indexOf("\\n") !== -1) pem = pem.replace(/\\n/g, "\n");
  return {
    pem:       pem.trim(),
    ...
    serviceId: (process.env.WEATHERKIT_SERVICE_ID || process.env.APPLE_NATIVE_BUNDLE || "").trim(),
  };
}
