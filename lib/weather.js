...
//        WEATHERKIT_SERVICE_ID = the registered WeatherKit Service ID / bundle id
//                                (falls back to APPLE_NATIVE_BUNDLE so no new var is needed)
...
// Make a .p8 private key tolerant of env-storage mangling: escaped "\n",
// base64-wrapped whole file (no PEM headers), or a single-line PEM whose
// internal newlines were collapsed. Returns a clean multi-line PKCS8 PEM.
function _normalizePem(s) {
  let p = String(s || "").trim();
  if (!p) return p;
  if (p.indexOf("\\n") !== -1) p = p.replace(/\\n/g, "\n");      // escaped → real newlines
  if (!/-----BEGIN/.test(p)) {                                    // maybe base64 of the whole .p8
    try { const dec = Buffer.from(p, "base64").toString("utf8"); if (/-----BEGIN/.test(dec)) p = dec; } catch (_) {}
  }
  // If headers exist but the base64 body lost its line breaks, re-wrap at 64.
  const m = /-----BEGIN ([A-Z0-9 ]+)-----([\s\S]*?)-----END \1-----/.exec(p);
  if (m) {
...
function _loadConfig() {
  ...
  if (raw.trim().startsWith("{")) {
    try {
      ...
      const pem = _normalizePem(j.private_key || j.privateKey || "");
      return {
        pem,
        ...
        serviceId: String(j.service_id || j.serviceId || process.env.WEATHERKIT_SERVICE_ID || process.env.APPLE_NATIVE_BUNDLE_ID || process.env.APPLE_NATIVE_BUNDLE || process.env.APPLE_CLIENT_ID || "").trim(),
      };
    } catch (_) { /* fall through to PEM form */ }
  }
  // Form A: raw PEM + separate id vars. The `sub` (Service ID) falls back to the
  // app's already-set bundle/audience vars (APPLE_NATIVE_BUNDLE_ID → APPLE_CLIENT_ID,
  // same chain server.js uses) so no new env var is needed when WeatherKit is
  // enabled on that identifier — respects the 20-var cap.
  return {
    pem:       _normalizePem(raw),
    keyId:     (process.env.WEATHERKIT_KEY_ID || "").trim(),
    teamId:    (process.env.APPLE_TEAM_ID || process.env.WEATHERKIT_TEAM_ID || "").trim(),
    serviceId: (process.env.WEATHERKIT_SERVICE_ID || process.env.APPLE_NATIVE_BUNDLE_ID || process.env.APPLE_NATIVE_BUNDLE || process.env.APPLE_CLIENT_ID || "").trim(),
  };
}
...
export async function getForecast({ lat, lng, tz = "UTC", dayYmd, startTime = null }) {
  ...
  return {
    ...
    atStart,
    advisory: {
      // Fire on any storm hour in-window OR a storm daily summary (the daily
      // rollup sometimes flags storms the per-hour codes don't) — safety-first.
      lightning: lightning || LIGHTNING_CODES.has(String(day.conditionCode)),
