...
function parseD0(line) {
  ...
  // Prefer an ACHIEVED time: finals, else prelim. Seed is an entry estimate — not a PR.
  let timeText = trim(slice(line, D0.finalsTime)), courseRaw = trim(slice(line, D0.finalsCourse));
  let timeSecs = parseSdifTime(timeText);
  if (timeSecs == null) {
    timeText = trim(slice(line, D0.prelimTime)); courseRaw = trim(slice(line, D0.prelimCourse));
    timeSecs = parseSdifTime(timeText);
  }
