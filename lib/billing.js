...
function _loadStripeConfig() {
  ...
  if (raw) {
    try {
      ...
      return {
        ...
        price_id_supporter_monthly: parsed.price_id_supporter_monthly || "",
  ...
  return {
    ...
    price_id_supporter_monthly: process.env.STRIPE_PRICE_ID_SUPPORTER_MONTHLY  || "",
...
const PRICE_SUPPORTER_MONTHLY = STRIPE_CFG.price_id_supporter_monthly;
// Price → tier resolution for checkout + webhook. Lesson + supporter are inert
// until their price_id is configured in Stripe. The SUPPORTER tier (eval #7) is
// an OPTIONAL "support SetForge" subscription that gates NOTHING — it grants no
// entitlements, so the swimmer side stays free-for-all (free-for-swimmers rule).
const PRICE_BY_TIER  = { coach: PRICE_COACH_MONTHLY, lesson: PRICE_LESSON_MONTHLY, supporter: PRICE_SUPPORTER_MONTHLY };
const TIER_BY_PRICE  = {};
...
if (PRICE_SUPPORTER_MONTHLY) TIER_BY_PRICE[PRICE_SUPPORTER_MONTHLY] = "supporter";
...
export function billingConfigState() {
  return {
    ...
    has_price_id_supporter: !!STRIPE_CFG.price_id_supporter_monthly,
...
export async function createCheckoutSession({ userSub, successUrl, cancelUrl, tier = "coach" }) {
  ...
  const wantTier = (tier === "lesson" || tier === "supporter") ? tier : "coach";
  const price = PRICE_BY_TIER[wantTier];
  if (!price) return { error: "no_price_id" };   // lesson/supporter inert until their price_id is configured
  ...
  // Supporter is an optional "chip in" subscription — no free trial (a trial on
  // a donation makes no sense). Coach/Lesson keep the 14-day trial.
  if (wantTier !== "supporter") subData.trial_period_days = TRIAL_DAYS;
  ...
  switch (type) {
    ...
    case "customer.subscription.updated": {
      ...
      if (["active", "trialing"].includes(status)) {
        // Resolve which tier this subscription grants: prefer the tier stamped on
        // the subscription metadata at checkout, else map the price id. Do NOT
        // default to coach — an unmapped/Dashboard-created/legacy price would
        // silently grant full coach to someone who bought a lesser tier.
        const metaTier = obj?.metadata?.tier;
        const priceId  = obj?.items?.data?.[0]?.price?.id;
        const resolvedTier = (metaTier === "coach" || metaTier === "lesson" || metaTier === "supporter") ? metaTier
                            : (priceId && TIER_BY_PRICE[priceId]) ? TIER_BY_PRICE[priceId]
                            : null;
        if (!resolvedTier) {
          console.warn(`[billing] subscription ${obj.id} active but tier unresolved (metaTier=${metaTier || "none"}, priceId=${priceId || "none"}) — NOT granting`);
          return { handled: type, status, action: "tier_unresolved_no_grant" };
        }
      ...
      // NOTE: past_due is intentionally NOT here — Stripe sets it on a single failed
      // renewal while dunning/Smart Retries continue, and the sub usually recovers to
      // active. Revoking on past_due would strip a still-paying customer mid-cycle.
...
export async function grantTier({ userSub, tier, source }) {
  ...
  const allowed = ["free", "coach", "lesson", "program", "supporter"];   // + supporter (eval #7, no entitlements)
...
export async function getBillingStatusFor(userSub) {
  ...
  const r = rows[0];
  const cfg = billingConfigState();
  const flags = {
    // Surface which tiers are actually buyable so the client can show/hide the
    // right upgrade buttons (coach / lesson / supporter). Inert tiers hide.
    billing_active:         cfg.active,
    has_price_id:           cfg.has_price_id,
    has_price_id_lesson:    cfg.has_price_id_lesson,
    has_price_id_supporter: cfg.has_price_id_supporter,
  };
  if (!r) return { tier: "free", ...flags };
  return {
    tier:                  r.tier || "free",
