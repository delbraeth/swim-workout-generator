...
export async function createCheckoutSession({ userSub, successUrl, cancelUrl }) {
  if (!BILLING_ACTIVE) return { skipped: "inactive" };
  if (!PRICE_COACH_MONTHLY) return { error: "no_price_id" };
  ...
  const session = await stripe.checkout.sessions.create({
    mode:       "subscription",
    customer:   customerId,
    line_items: [{ price: PRICE_COACH_MONTHLY, quantity: 1 }],
  ...
  switch (type) {
    ...
    case "customer.subscription.updated": {
      ...
      if (["active", "trialing"].includes(status)) {
        await grantTier({ userSub, tier: "coach", source: `stripe_sub_${obj.id}` });
        return { handled: type, status, action: "granted_coach" };
      }
      // Revoke only on TERMINAL states. `past_due` is transient — Stripe
      // sets it on the first failed renewal retry while dunning is still
      // running and the customer may still pay; revoking there would lock
      // a paying coach out mid-cycle on a card hiccup. Treat it as soft
      // (no change); the eventual `unpaid`/`canceled`/`deleted` event
      // does the revoke if dunning ultimately fails.
