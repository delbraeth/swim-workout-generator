...
import {
  ...
  dbListGuardianSubsForSwimmer, dbGetDisplayNameForSub, dbGetSettings,
} from "../db.js";
...
// D2 — fan a non-responder's RSVP reminder out to their guardians, IF the swimmer
// opted into guardian involvement (settings.extra.allow_guardian_rsvp) AND the
// guardian hasn't muted swimmer alerts (settings.extra.mute_swimmer_alerts).
// Deduped per (guardian, target:child) so siblings on one event don't collapse to
// a single push. Best-effort: never throws.
async function _remindGuardians(t, dedupKind, swimmerSub) {
  let consented = false;
  try { const cs = await dbGetSettings(swimmerSub); consented = !!(cs && cs.allow_guardian_rsvp === true); } catch (_) {}
  if (!consented) return 0;
  let guardianSubs = [];
  try { guardianSubs = await dbListGuardianSubsForSwimmer(swimmerSub); } catch (_) { return 0; }
  if (!guardianSubs.length) return 0;
  let childName = null;
  try { childName = await dbGetDisplayNameForSub(swimmerSub); } catch (_) {}
  const who = childName || "your swimmer";
  const gDedup = `${dedupKind}_guardian`;
  const gTarget = `${t.id}:${swimmerSub}`;   // per-child key
  let n = 0;
  for (const g of guardianSubs) {
    let muted = false;
    try { const gs = await dbGetSettings(g); muted = !!(gs && gs.mute_swimmer_alerts === true); } catch (_) {}
    if (muted) continue;
    let fresh;
    try { fresh = await dbMarkNotifiedOnce(gDedup, gTarget, g); } catch (_) { fresh = false; }
    if (!fresh) continue;
    try {
      const res = await sendPushToUser(g, {
        title: `📣 RSVP needed — ${who}`,
        body: `${who} hasn't responded to ${t.name} on ${t.date}${t.start_time ? ` at ${t.start_time}` : ""}.`,
        url: "/parent",
      });
      if (res && res.sent > 0) n++;
      else if (!res || (!res.no_subs && !res.skipped)) await dbUnmarkNotifiedOnce(gDedup, gTarget, g);
    } catch (_) { await dbUnmarkNotifiedOnce(gDedup, gTarget, g); }
  }
  return n;
}
...
export async function runWeatherAdvisorySweep() {
  ...
  for (const ev of events) {
    ...
    for (const sub of subs) {
      ...
      try {
        // sendPushToUser SWALLOWS per-endpoint delivery errors (never throws) and
        // returns { sent, pruned }. So check `sent`: if nothing was delivered to a
        // user who HAS live subscriptions (sent===0, not the no-subs/inactive case),
        // release the once-marker so a later tick retries instead of suppressing forever.
        const res = await sendPushToUser(sub, {
          title: `${label} — ${ev.name}`,
        ...
        if (res && res.sent > 0) pushed++;
        else if (!res || (!res.no_subs && !res.skipped)) await dbUnmarkNotifiedOnce(kind, ev.id, sub);
      } catch (e) {
        await dbUnmarkNotifiedOnce(kind, ev.id, sub);   // list-query threw → retry next tick
...
export async function runRsvpReminderSweep() {
  ...
  for (const t of targets) {
    ...
    for (const sub of audience) {
      ...
      // D2 — fan this non-responder's nudge out to their guardians. Own dedup, so
      // it's independent of whether the swimmer's own reminder already fired.
      try { gReminded += await _remindGuardians(t, dedupKind, sub); } catch (_) {}
      let fresh;
      ...
      try {
        // sendPushToUser never throws on delivery failure — check `sent` (see weather sweep).
        const res = await sendPushToUser(sub, {
          title: `📣 RSVP needed — ${t.name}`,
          body: `${t.name} on ${t.date}${t.start_time ? ` at ${t.start_time}` : ""}: you haven't responded yet.`,
          url: "/assigned",
        });
        if (res && res.sent > 0) reminded++;
        else if (!res || (!res.no_subs && !res.skipped)) await dbUnmarkNotifiedOnce(dedupKind, t.id, sub);
      } catch (e) {
        await dbUnmarkNotifiedOnce(dedupKind, t.id, sub);   // list-query threw → retry next tick
        console.warn(`[notify] rsvp reminder push failed: ${e.message}`);
      }
    }
  }
  if (reminded || gReminded) console.log(`[notify] rsvp-reminder sweep: ${reminded} swimmer + ${gReminded} guardian reminder(s)`);
  return { reminded, guardian_reminded: gReminded };
}
