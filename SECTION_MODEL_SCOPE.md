...
- **B2 — ✅ SHIPPED 2026-05-30.** Guarded EVERY `block.sets` consumer for dryland FIRST, then added the insert UI. Guards: pace-scale helper (~13754) + handleApplyPace (~26349) `Array.isArray(b.sets)?...:b`; two print views (`(block.sets||[]).map`); Run mode allSets + per-block render + tap-index reduce (`(block.sets||[])`, `b.sets?b.sets.length:0`); `workoutToText` renders dryland as an exercise list; **save-to-history (~27044) + clone/repeat (~27173) persist `kind/placement/exercises` not sets** (round-trips). Already-guarded: intensity dashboard, save-preview, calcEstimatedMin (continue). Safe-by-construction: generate-time rescales (no dryland at gen), single-section regen + single-set editors (targeted swim block only), snapshot (swim-only). UI: "🏋 + Add dryland" below the workout → pick preset + pre/post → inserts into `workout.blocks`; DrylandBlock has a remove ✕. Verified: babel OK, engine byte-identical, calcEstimatedMin + copy-text dryland round-trip clean. **Known B2→B3 gap (crash-safe, not pretty): Run mode shows a dryland block with no set-cards; print shows its header but not the exercises** (copy-text DOES show them).
- **B3 — ✅ SHIPPED 2026-05-30. PART B COMPLETE.** Run-mode: dryland blocks render a tick-off **checklist** (tap to check, strikethrough; `drylandChecked` Set state) instead of pace cards. Multi-pace print (matrix + per-lane): dryland renders its **exercise list**. **Plus a server fix that also unblocked Part A:** `validateLoggedEntry` (server.js ~1279) required EXACTLY 4 fixed-order swim sections — so it rejected BOTH skipped-section workouts (Part A) AND dryland (Part B) with 400 on `/api/log-workout`. Relaxed to: non-empty blocks, exactly one `main`, swim blocks have non-empty `sets`, dryland blocks have an `exercises` array. Reports already exclude dryland (totalYards:0; db.js reporting guards with `Array.isArray(b.sets)`). Verified: node --check server.js + babel + engine byte-identical.

**PART A + PART B BOTH COMPLETE.** Section model is fully shipped: selectable swim sections (skip warmup/drill/cooldown, target-holding) + dryland blocks (add preset pre/post, render/run/print/save/copy all dryland-aware). Future polish (not blocking): ad-hoc dryland authoring (edit exercises inline), curated bank expansion, dryland in the WeekView intent flow.
