...
import { equipmentForSet, getEquivalents, makeDrylandBlock, makeEntryId, minYardsForType, parseIntent, rescaleBlocksForPace } from "./lib/workout-helpers.js";
    ...
    // 🔱 Format a workout for export to TrainingPeaks (the tri "bridge"). Leads with the
    // planned metrics TP wants — total distance in METERS + duration — then a clean
    // structured body to paste into the workout's description. The athlete sets TP's
    // planned distance/duration from the header line; the body documents the set.
    function workoutToTrainingPeaks(workout, meta, paceInput, unit = "yds") {
      if (!workout) return "";
      const estMin  = calcEstimatedMin(workout.blocks);
      const totalU  = workout.totalYards || 0;
      const meters  = unit === "m" ? totalU : Math.round(totalU * 0.9144);
      const dur     = `${Math.floor(estMin / 60)}:${String(estMin % 60).padStart(2, "0")}:00`;
      const lines = [];
      lines.push(`${meta.label} Swim — ${totalU.toLocaleString()} ${unit} (~${estMin} min)`);
      lines.push(`Planned distance: ${meters.toLocaleString()} m   ·   Planned duration: ${dur}`);
      if (paceInput) lines.push(`Pace: ${paceInput}/100${unit === "m" ? " m" : ""}`);
      if (workout.phase && PHASES[workout.phase]) lines.push(`Training phase: ${PHASES[workout.phase].label}`);
      lines.push("");
      for (const block of workout.blocks) {
        if (!Array.isArray(block.sets)) continue;   // TP swim export skips dryland blocks
        const rounds = block.rounds || 1;
        lines.push(`${block.name.toUpperCase()} — ${block.totalYards.toLocaleString()} ${unit}${rounds > 1 ? ` (×${rounds} rounds)` : ""}`);
        for (const set of block.sets) {
          const showIvl = set.interval && !/^no interval/i.test(set.interval);
          lines.push(`  ${set.reps} x ${set.dist} — ${set.desc}${showIvl ? ` @ ${set.interval}` : ""}`);
        }
        lines.push("");
      }
      lines.push("— Paste into the TrainingPeaks workout description; set planned distance + duration from the header. Generated by SetForge.");
      return lines.join("\n").trimEnd();
    }
              ...
              <div>
                ...
                <div className="screen-only" data-tour="step-celebrate" style={{
                  ...
                  <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
                    ...
                    <button
                      onClick={() => {
                        const text = workoutToTrainingPeaks(workout, meta, paceInput, unit);
                        navigator.clipboard.writeText(text).then(() => {
                          setTpFlash(true);
                          setTimeout(() => setTpFlash(false), 2000);
                        });
                      }}
                      style={{
                        padding: "10px 16px", borderRadius: 8,
                        border: `1px solid ${tpFlash ? "var(--color-positive)" : "var(--color-border-strong)"}`,
                        background: tpFlash ? "#052e16" : "var(--color-bg)",
                        color: tpFlash ? "#86efac" : "var(--color-text)",
                        fontSize: 13, fontWeight: 700, cursor: "pointer",
                        display: "inline-flex", alignItems: "center", gap: 6,
                        transition: "all 0.2s",
                      }}
                      title="Copy a TrainingPeaks-ready version (planned distance + duration + structured body) to paste into a TP swim workout.">
                      {tpFlash ? "✓ Copied for TP!" : "🔱 TrainingPeaks"}
                    </button>
                    )}
