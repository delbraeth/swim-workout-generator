...
import { FacilityMap } from "./FacilityMap.jsx";
    ...
    export function TeamFacilitiesSection({ teamId, canWrite }) {
      ...
      const [editId, setEditId] = React.useState(null);   // facility id being edited
      const [editForm, setEditForm] = React.useState(emptyF());
      ...
      const startEdit = (f) => {
        setEditId(f.id);
        setEditForm({
          name: f.name || "", course: f.course || "", lanes: f.lanes == null ? "" : String(f.lanes),
          is_primary: !!f.is_primary,
          line1: (f.address && f.address.line1) || "", city: (f.address && f.address.city) || "",
          region: (f.address && f.address.region) || "", postal_code: (f.address && f.address.postal_code) || "",
        });
        setMsg(null);
      };
      const cancelEdit = () => { setEditId(null); setEditForm(emptyF()); setMsg(null); };
      const saveEdit = async () => {
        if (!editForm.name.trim()) { setMsg("Facility name required."); return; }
        setBusy(true); setMsg(null);
        try {
          const res = await fetch(`${API_BASE}/teams/${teamId}/facilities/${editId}`, { method: "PATCH", headers: { "Content-Type": "application/json", ...csrfHeaders() }, body: JSON.stringify(body(editForm)) });
          const j = await res.json().catch(() => ({}));
          if (!res.ok) throw new Error(j.error || `HTTP ${res.status}`);
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 10 }}>
              {list.map(f => {
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 6, padding: "10px", background: "var(--color-bg)", borderRadius: 6 }}>
                  {facilityFields(editForm, updEdit)}
                  <div style={{ display: "flex", gap: 8 }}>
                    <button onClick={saveEdit} disabled={busy || !editForm.name.trim()} style={{ padding: "6px 14px", borderRadius: 6, border: "none", background: (busy || !editForm.name.trim()) ? "var(--color-border)" : "var(--color-primary)", color: "#fff", fontSize: 12, fontWeight: 700, cursor: (busy || !editForm.name.trim()) ? "not-allowed" : "pointer" }}>{busy ? "Saving…" : "Save changes"}</button>
                    <button onClick={cancelEdit} disabled={busy} style={{ padding: "6px 10px", borderRadius: 6, border: "1px solid var(--color-border)", background: "transparent", color: "var(--color-text-dim)", fontSize: 12, cursor: "pointer" }}>Cancel</button>
                  </div>
                </div>
                ) : (
                ...
                {mappable && isOpen && editId !== f.id && <FacilityMap facility={f} />}
                </React.Fragment>
                );
              })}
            </div>
