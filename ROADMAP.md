...
Last refreshed: **2026-05-23** (post UGC coach-sets scope lock; View-as v2 stale entry cleaned up)
...
- **Coach-authored workout sets (UGC into engine)** — spec locked 2026-05-23 in repo `UGC_COACH_SETS_SCOPE.md`. Coaches author sets via form OR snapshot of a generated workout; visibility tiers private / team-I-pick / public-with-admin-moderation; UGC extends the existing bank with `author_sub` column (one unified picker, fav/disfav system Just Works); team-shared sets auto-propagate to swimmers mirroring [[swim-generator-favorites-prop-v13]]. 6 phases (A migration+backfill → B picker-reads-DB → C authoring → D team sharing → E public+moderation → F smoke+tag). ~27-38h. Recommendation in spec §10: ship Phases A+B as pure tech-debt cleanup BEFORE coach demand arrives (unblocks bank edits without redeploy, decoupled from UGC user-visible work). Source: [[swim-generator-ugc-coach-sets-scope]]
