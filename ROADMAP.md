...
Last refreshed: **2026-05-24** (UGC Phase A shipped: migration 030 + sync_bank.mjs --import; phase B next)
...
- **Coach-authored workout sets (UGC into engine)** — spec locked 2026-05-23 in repo `UGC_COACH_SETS_SCOPE.md`. **Hybrid architecture**: bank source-of-truth = DB; client reads `public/bank-generated.js` (committed artifact regenerated by manual CLI `tools/sync_bank.mjs --export`); UGC sets fetched per-session via `/api/bank/my-overlay`. Canonical rows carry an `in_export` DRAFT/LIVE flag. 7 phases (A migration+backfill → B export tool+staging UI → C UGC overlay → D authoring → E team sharing → F public+moderation → G smoke+tag). ~32-45h. **Status:**
  - **Phase A** ✅ SHIPPED 2026-05-24 (tag `ugc-phase-a` pending Cap'n's prod backfill). Migration 030 + tools/sync_bank.mjs --import. Dry-run: 1311 options + 3276 sets across 3 pool modes. No client behavior change. Source: [[swim-generator-ugc-phase-a]]
  - **Phase B** — NEXT (~6-9h): export tool + bank-generated.js + admin canonical-bank tab with DRAFT/LIVE staging UI. Completes the standalone "bank in DB" foundation before any UGC user-visible work.
  - **Phases C-G** — demand-driven; ship after first paying coach asks for UGC. Source: [[swim-generator-ugc-coach-sets-scope]]
...
| Date | Tag | What |
|---|---|---|
| 2026-05-24 | `ugc-phase-a` (pending prod backfill) | UGC Phase A: migration 030 (bank_options + bank_sets + team_shares + reviews + in_export DRAFT/LIVE flag) + tools/sync_bank.mjs --import. Dry-run verified 1311 options + 3276 sets across 3 pool modes. Patched a missing s_xxxxxx ID on a MAIN_OPTIONS set found during inventory. Internal-only ship; client picker still reads JS constants until Phase B. |
